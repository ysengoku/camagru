...
| Login / logout | `GET/POST /login`, `POST /api/logout` | `LoginService` |
...
- `app/src/Services/auth/` — `SignupService`, `LoginService`, `ForgotPasswordService`, `ResetPasswordService`, `AuthInputValidator`.
- `app/src/core/SessionStore.php` — typed wrapper around PHP's native `$_SESSION`.
- `app/src/DTO/SignupData.php`, `ServiceResult.php`, `SessionKey.php`.
