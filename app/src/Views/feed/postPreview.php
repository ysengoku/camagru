...
if (!function_exists('render_post_preview')) {
    function render_post_preview(PostData $post, bool $interactive = false): string {
        $header    = render_post_header($post->author_name, $post->author_avatar, $post->created_at);
        $reactions = render_post_reactions($post->id, $post->likes_count, $post->is_liked_by_current_user, $post->comments_count, $interactive);
